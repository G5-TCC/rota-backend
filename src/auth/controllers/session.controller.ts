import { Controller, Get, Delete, Param, UseGuards, Request, UseInterceptors } from '@nestjs/common';
...
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: 'Acesso negado. JWT inválido ou ausente.' })
...
export class SessionController {
  ...
  @ApiOkResponse({ description: 'Lista de sessões retornada com sucesso.', type: [SessionResponse] })
  ...
  async revokeAllOthers(@Request() req: any) {
    return this.sessionService.revokeAllOthers(req.user.sub, req.user.sessionId);
  }
  ...
  @ApiForbiddenResponse({ description: 'Você não tem permissão para revogar esta sessão.' })
