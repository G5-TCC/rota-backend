...
describe('PaymentService', () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        },
        {
          ...
          useValue: { user: { findUnique: jest.fn() }, transaction: { create: jest.fn() } },
        },
        {
  ...
  it('should create a checkout for a plan', async () => {
    prisma.user.findUnique.mockResolvedValue({ id: 'u1', email: 'a@b.com' } as any);
    ...
    gateway.createSubscription.mockResolvedValue({ id: 'ch1', url: 'http://url' });
    ...
    expect(gateway.createProduct).toHaveBeenCalledWith('PRO', 'Plano PRO', 4990);
  });
});
